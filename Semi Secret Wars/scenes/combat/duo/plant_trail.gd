...
const PLANT_ART := preload("res://assets/sprites/Verdant_Path.png")
...
func _draw() -> void:
	for offset in _plant_offsets:
		...
		BattleFX.draw_burst(self, PLANT_ART, offset, PLANT_MARK_RADIUS * 2.6, 1.0)
		if _flash_t > 0.0:
			var p := 1.0 - _flash_t / FLASH_TIME
			BattleFX.draw_burst(self, PLANT_ART, offset, plant_radius * 2.0 * p, 1.0 - p)
