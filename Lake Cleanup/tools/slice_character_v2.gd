## Cuts the second angler's sprite sheet into a catalogue.
##
## Different shape from tools/slice_character.gd, because the source is different: this one
## arrived as twelve flat, hand-trimmed strips — one per animation and compass direction,
...
##
## Four real directions, not three plus a mirror: the art draws north, south, east and west
## each on their own, so there is nothing here to flip at draw time.
##
## No head mark. The old sheet needed one because the game draws its own hat on top of a
## bare head; this one already has its hat painted in, so nothing has to be told where to
## put a second one.
##
##   godot --headless --path . --script res://tools/slice_character_v2.gd
extends SceneTree

## Where the psd-extract skill left the twelve strips.
const SOURCE_DIR := "res://art_source/character_v2_extracted/"
...
const OUT_PNG := "res://assets/character_v2.png"
const OUT_JSON := "res://assets/character_v2.json"
const DEBUG_PNG := "res://assets/sliced_character_v2.png"
