...
### Derived Match Ratings (`scripts/match_ratings.gd`, Milestone 20)

Every player-vs-player contest reads a **named rating** from `MatchRatings` (static), so the same blend isn't re-derived from scattered constants across the engine. All ratings are formulas over the existing 5 stats, position-adjusted via `PositionCompatibility.get_effective_stats()` and scaled by in-match Condition (`condition_multiplier`, the `CONDITION_FLOOR_MULTIPLIER`=0.5 formula below). No new stored attributes — "derive from the 5 stats" (Designer decision, M20):
- `tackling` = `strength×DEFENSE_STRENGTH_WEIGHT(0.6) + speed×DEFENSE_SPEED_WEIGHT(0.4)` — used by tackles **and** interceptions.
- `retention` = `strength×RETENTION_STRENGTH_WEIGHT(0.5) + speed×RETENTION_SPEED_WEIGHT(0.3) + passing×RETENTION_PASSING_WEIGHT(0.2)` — a carrier resisting a tackle.
- `finishing` = Kick; `passing_skill` = Pass; `keeper_rating` = Strength; `composure` = Pass (reserved for decision noise / penalty conversion in later milestones).
...
- **Carrier decisions (`MatchDecisionEngine._make_decision`):** every `DECISION_INTERVAL` (randomized 0.8–1.6s) while a player holds the ball, they evaluate shoot / pass / dribble:
  - **Shoot** when within `SHOOTING_RANGE` = 300 world units of the real goal mouth (continuous distance, not a discrete zone). Chance per decision = `SHOT_CHANCE_BASE` (0.14) + proximity × `SHOT_CHANCE_PROXIMITY_BONUS` (0.55) + `SHOT_CHANCE_OPEN_BONUS` (0.25, if no defender is within `PRESSURE_RANGE`=90 — a clear sight of goal), proximity = `1 - dist/SHOOTING_RANGE`. Widened/made more eager than the original pass-first tuning so a real chance gets taken rather than passed up.
  - **Pass** — candidate receiver picked by weighted-random among fielded teammates within `PASS_MIN_DISTANCE`–`PASS_MAX_DISTANCE` (60–480 units, trimmed from an original 750 so a genuine long switch is rare rather than routine), weight = `(1/distance^DISTANCE_DECAY_EXPONENT) × directness_factor` where `DISTANCE_DECAY_EXPONENT` = 1.6 (steep proximity preference) and directness is a *ratio* (`FORWARD_DIRECTNESS_WEIGHT`=2.5 × forward_progress/distance, using the side's real current attack direction — not raw world-unit progress, so distant options aren't inflated), with level-or-behind candidates multiplied down by `BACKWARD_PASS_PENALTY`=0.12. Chance = `clamp((PASS_CHANCE_BASE (0.55) + PASS_CHANCE_PRESSURE_BONUS (0.25 if a defender is within PRESSURE_RANGE=90) + PASS_CHANCE_OWN_HALF_BONUS (0.25 while the carrier is in their own half) − OPEN_LANE_PASS_REDUCTION (0.35 if the nearest opponent is beyond OPEN_LANE_RANGE=170, i.e. a real lane to goal is open)) × (0.5 + carrier.passing/100), 0.05, 0.9)`.
  - **Dribble** — the default when neither fires: `PlayerMovementSystem` already drives the carrier forward every frame.
- **Tackles (`MatchDecisionEngine._update_tackle_pressure`):** the nearest real opposing defender within `TACKLE_RANGE` = 45 rolls a contest every `TACKLE_ATTEMPT_INTERVAL` = 1.4s while in range: `p_defender_wins = defender_rating / (defender_rating + carrier_rating)`, both condition-scaled — defender rating = `MatchRatings.tackling`, carrier retention rating = `MatchRatings.retention` (see Derived Match Ratings above). A win hands the ball straight to the defender (already adjacent — no separate loose-ball phase needed).
- **Pass interception (`MatchDecisionEngine._check_interception`):** any real opponent within `INTERCEPTION_RADIUS` = 55 world units of the pass lane can cut it out; the nearest/highest-rated candidate (rated by `MatchRatings.tackling`) rolls `p_intercept = defender_rating×closeness / (defender_rating×closeness + passer_rating × PASS_LANE_SAFETY)`, where `passer_rating = MatchRatings.passing_skill` and `PASS_LANE_SAFETY` = 1.3 (favors the passer). A miss lands the pass at the receiver; a hit diverts the ball to the interception point along the lane.
...
- **Shot resolution — xG model (`MatchDecisionEngine._take_shot` / `_shot_xg`, Milestone 20):** replaces the old distance-only `SHOT_QUALITY` lerp. Two stages:
  - **Chance quality (xG, 0–1)** from the shot's *real geometry*, independent of who's shooting (so the accumulated xG stat means "chance quality," the standard xG sense): `xg = XG_MAX(0.9) × angle_norm × dist_factor`, then multiplied down by lane blockers. `angle_norm = clamp(subtended_goal_angle / XG_ANGLE_REF(1.0 rad), 0, 1)` — the angle subtended between the two goalposts (mouth = `GOAL_WIDTH_RATIO`=0.16 of grass height, matching `pitch_goals.gd`), so wide *and* far shots both narrow it. `dist_factor = exp(-distance / XG_DISTANCE_SCALE(320))`. Each outfield opponent (not the GK) standing across the shot lane — within `XG_BLOCK_RADIUS`=38 of the shooter→goal segment, `0.05<t<0.95` — multiplies xG by `XG_BLOCK_FACTOR`=0.7, capped at `XG_MAX_BLOCKERS`=3. Floored/capped to `XG_MIN`(0.02)…`XG_MAX`(0.9).
  - **Conversion to a goal:** `p_goal = clamp(xg × finish_mult / keeper_mult, XG_MIN, P_GOAL_CAP=0.9)`, where `finish_mult = FINISH_MULT_BASE(0.7) + FINISH_MULT_SPAN(0.5)×(finishing/100)` and `keeper_mult = KEEPER_MULT_BASE(0.75) + KEEPER_MULT_SPAN(0.45)×(keeper_rating/100)` (`finishing`/`keeper_rating` from `MatchRatings`). Equal-quality finisher and keeper ⇒ `p_goal ≈ xg`; a better finisher vs. a weaker keeper pushes it above, and vice versa. A miss is "SAVED" `SAVED_FRACTION` (0.6) of the time, "OFF_TARGET" otherwise — flavor-only split. Shot *frequency* (the `_make_decision` shoot gate) is unchanged this milestone.
- **Match statistics (Milestone 20):** `LiveMatchState` accumulates per side — **shots** (every attempt), **shots on target** (GOAL or SAVED), and **xG** (sum of per-shot chance quality); **possession %** comes from the time-based `TeamMatchState.possession_pct`. Surfaced live in the Live Match sidebar and on the Match Result screen (`MatchResult` carries them through). These are the objective yardstick for judging match believability from M20 onward.
...
- **Movement (`scripts/player_movement_system.gd`):** formation slot is the base reference. Ball carriers dribble forward (`DRIBBLE_ADVANCE_STEP`=75, `DRIBBLE_WANDER`=12, `PITCH_EDGE_MARGIN`=20 clamp); any player tracks the ball within `BALL_TRACKING_RADIUS`=380. Off-ball attackers push forward from their formation slot by a per-category amount (`ATTACK_PUSH_BY_CATEGORY`: DEF 200 / MID 280 / FWD 310 — bigger for further-forward roles, committing real numbers into the attacking third), plus a small ball-side lean (`ATTACK_BALL_PULL`=0.06) and a width stretch off the pitch's vertical center (`ATTACK_WIDTH_STRETCH`=1.15) so wide players hug the touchline; separation from teammates uses `SEPARATION_RADIUS`=110/`SEPARATION_STRENGTH`=55. The whole team also steps further forward as the ball carrier advances into the attacking half (`ATTACK_LINE_MAX_PUSH`=170, mirrors the defensive-line push below). When the carrier is deep and wide (`CROSS_ZONE_DEPTH`=320, `CROSS_ZONE_WIDE_Y`=140), other attackers abandon the normal shape and crowd the box instead (`BOX_CRASH_DEPTH`=90, spread `BOX_CRASH_WIDTH_SPREAD`=110 across near-post/center/far-post). Defenders press — only the single nearest non-GK defender presses tightly, `PRESSING_RADIUS`=260 base, growing up to `PRESSING_RADIUS_ATTACK_BONUS`=160 extra the further forward the team's press line has already pushed — or man-mark within `MARKING_RADIUS`=320 at `MARKING_BLEND`=0.5, shaded goal-side of the mark by `MARK_GOAL_SIDE_BIAS`=35. The defending team's own reference shape pushes forward as play moves deeper into their attacking half (`DEFENSIVE_LINE_MAX_PUSH`=160). The goalkeeper stays within a fixed radius of their own goal line/box and is excluded from pressing/marking. Movement is steered with an acceleration cap and eases down near the target instead of snapping to a fresh heading every frame, removing frame-to-frame jitter from a heuristic target that flip-flops (e.g. switching who presses vs. marks). `BASE_MOVEMENT_SPEED` = 200 units/sec scaled by `player.speed/100`.
