...
# art/assets.json. LayerDiffuse emits a true alpha channel in one pass, so there
# is no background-removal guesswork.
#
...
function New-Graph($asset, [int]$seed, [bool]$transparent, [string]$refFile) {
    $w, $h = $asset.gen
    ...
    $g = [ordered]@{
        ...
        "3" = @{ class_type = "CLIPTextEncode";        inputs = @{ text = (Get-Setting $asset 'negative' ''); clip = @("1", 1) } }
...
foreach ($asset in $assets) {
    ...
    for ($v = 1; $v -le $variants; $v++) {
        ...
        if (-not $useModel) {
            ...
            if ($DryRun) {
                $graph = New-Graph $asset $seed $transparent $refFile
